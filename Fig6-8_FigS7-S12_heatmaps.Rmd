---
title: "Figure 6-8 and S7-S12: Heatmaps"
...
### Figure S7: Archaea diversity (V3-V5: archaea only)
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Select data
...
p1 <- ggplot(df1, aes(TempSubstrate, Genus)) +
     ...
     scale_fill_manual(values=c("#d94801","#f16913","#fd8d3c","#fdae6b","#fdd0a2","#feedde"), drop=FALSE) +
...
ggsave(filename="output/FigureS7_Archaea_V35.pdf", plot=p, width=240, height=90, units = "mm", useDingbats=FALSE, limitsize=FALSE)
```

### Figure 6 and S8: Archaea diversity (V4: archaea only)
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Select data
...
df <- amp_subset_taxa(df, tax_vector = c("g__Methanothermobacter"), normalise = FALSE)
...
# Combine p1 and p2
p1 <- p1 + theme(legend.position = "none")
p <- p1 + p2 + plot_layout(ncol=2,widths = c(10,17))

ggsave(filename="output/Figure6_Archaea_V4.pdf", plot=p, width=260, height=90, units = "mm", useDingbats=FALSE, limitsize=FALSE)
...
ggsave(filename="output/FigureS8_Archaea_ASV_V4.pdf", plot=p3, width= 160, height=250, units = "mm", useDingbats=FALSE, limitsize=FALSE)

# Calculate Archaeal fraction
df <- amp_subset_samples(d4n, Primary_substrate %in% c("Food waste","Industrial","Manure","Wastewater sludge"))
df <- amp_subset_samples(df, Temperature_range %in% c("Thermophilic", "Mesophilic"), normalise = TRUE)       
df <- amp_subset_taxa(df, tax_vector = c("k__Archaea"), normalise = FALSE)

df1 <- amp_heatmap(df,
            group_by = c("Primary_substrate","SampleID"),
...
df1 <- t(df1)
write.csv(df1, file="output/Archaea_fraction_V4.txt")
```
...
ggsave(filename="output/Figure7_Syntrophes_V13.pdf", plot=p, width=220, height=80, units = "mm", useDingbats=FALSE, limitsize=FALSE)
```

### FIGURE S9: Syntrophes (V4)
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Define Syntrophes
Syntrophes <- c("g__Syntrophomonas","g__Syntrophothermus", "g__Pelotomaculum", "g__Tepidimicrobium", "g__Thermacetogenium",
                "g__Syntrophaceticus", "g__Tepidanaerobacter", "g__Syntrophus", "g__Syntrophobacter", "g__Smithella", 
...
ggsave(filename="output/FigureS9_Syntrophes_V4.pdf", plot=p, width=240, height=80, units = "mm", useDingbats=FALSE, limitsize=FALSE)
```

### FIGURE S10: Species-level diversity of selected syntrophs (V1-V3)
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Select data
...
ggsave(filename="output/FigureS10_Syntroph_species_V13.pdf", plot=p, width=180, height=180, units = "mm", useDingbats=FALSE, limitsize=FALSE)
```
...
ggsave(filename="output/Figure8_Filaments_V13.pdf", plot=p, width=220, height=90, units = "mm", useDingbats=FALSE, limitsize=FALSE)
```


### Figure S11: Filaments (V4)
```{r, echo=FALSE, message=FALSE, warning=FALSE}
Filaments <- c("g__Ca_Microthrix", "g__Leptothrix","g__Sphaerotilus","g__Ca_Villigracilis","g__Trichococcus",
               "g__Thiothrix","g__Ca_Promineofilum","Haliscomenobacter","g__Gordonia","g__Sarcinithrix","g__Ca_Amarolinea","g__Kouleothrix","g__Ca_Alysiosphaera",
               "g__Nocardioides","g__midas_g_1668","g__Anaerolinea","g__Ca_Caldilinea","g__Ca_Hadersleviella","g__midas_g_344",
...
ggsave(filename="output/FigureS11_Filaments_V4.pdf", plot=p, width=210, height=90, units = "mm", useDingbats=FALSE, limitsize=FALSE)
```


### FIGURE S12: Species-level diversity of selected filaments (V1-V3)
```{r, echo=FALSE, message=FALSE, warning=FALSE}
#Select data
df <- amp_subset_samples(d13n, Primary_substrate %in% c("Wastewater sludge"))
df <- amp_subset_samples(df, Temperature_range %in% c("Mesophilic"))
...
ggsave(filename="output/FigureS12_Filaments_species_V13.pdf", plot=p, width=160, height=155, units = "mm", useDingbats=FALSE, limitsize=FALSE)
```